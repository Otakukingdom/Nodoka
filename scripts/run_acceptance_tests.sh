...
if command -v vlc &> /dev/null; then
    ...
    echo "  ✓ VLC is available: $VLC_VERSION"
    VLC_AVAILABLE=true
else
    echo "  ⚠ VLC not available - some tests may be skipped"
    VLC_AVAILABLE=false
fi
echo ""
...
if cargo clippy --all-targets --all-features -- -D warnings > "${RESULTS_DIR}/clippy_${TIMESTAMP}.txt" 2>&1; then
    ...
    echo "  ✗ Clippy found issues - see ${RESULTS_DIR}/clippy_${TIMESTAMP}.txt"
...
if cargo build --release > "${RESULTS_DIR}/build_${TIMESTAMP}.txt" 2>&1; then
    if grep -i "warning.*dead_code" "${RESULTS_DIR}/build_${TIMESTAMP}.txt"; then
        echo "  ✗ Dead code detected - must be removed"
        exit 1
    else
        echo "  ✓ No dead code found"
    fi
else
    echo "  ⚠ Build had warnings/errors - see ${RESULTS_DIR}/build_${TIMESTAMP}.txt"
fi
echo ""
...
if cargo test --lib -- --nocapture > "${RESULTS_DIR}/unit_${TIMESTAMP}.txt" 2>&1; then
    ...
    echo "  ✗ Unit tests failed - see ${RESULTS_DIR}/unit_${TIMESTAMP}.txt"
...
for category_info in "${categories[@]}"; do
    ...
    if cargo test --test "$test_file" -- --nocapture > "${RESULTS_DIR}/${test_file}_${TIMESTAMP}.txt" 2>&1; then
        ...
        CATEGORY_RESULTS="${CATEGORY_RESULTS}\n  ✓ $category_name: $PASSED tests passed"
    else
        echo "    ✗ Category failed - see ${RESULTS_DIR}/${test_file}_${TIMESTAMP}.txt"
